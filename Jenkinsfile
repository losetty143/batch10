node {
    try {
        ...
        stage('checkout'){
            git 'https://github.com/losetty143/batch10.git'
        }
    ...
    try {
        ...
        stage('Sonar Scan'){
           ...
           sh "mvn sonar:sonar -Dsonar.host.url=http://34.71.167.34:9000/ -Dsonar.login=admin -Dsonar.password=Rajsasi@143"
        }
    ...
    echo "RESULT: ${currentBuild.result}"
     try {
        ...
        //deploy adapters: [tomcat7(credentialsId: 'TomcatCred', path: '', url: 'http://34.136.39.153:8080/')], contextPath: 'index2', war: '**/*.war'
        //sh 'scp /var/lib/jenkins/workspace/CaseStudy_Final_Pipeline/target/bootcamp-0.0.1-SNAPSHOT.jar root@test-vm1:/usr/local/bootcamp/bootcamp.jar'
        //sh 'ssh root@10.128.0.13  java -jar /usr/local/bootcamp/bootcamp.jar'
        ansiblePlaybook installation: 'ansible-2.9.22', inventory: '/etc/ansible/hosts', playbook: '/var/lib/jenkins/workspace/application.yml'
